// pages/api/nachrichten/restore.js
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!threadId && !auftragId) return res.status(400).json({ error: "threadId ODER auftragId nötig" });

    const client = await clientPromise;
    const db = client.db(process.env.MONGODB_DB);

    const filter = buildFilter({ threadId, auftragId });
    const update = { $set: { deleted: false }, $unset: { deletedAt: "" } };
